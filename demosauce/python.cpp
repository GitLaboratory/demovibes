 ...
 * stuff changed for scenemusic by maep 
 *
...
{
  printf("WARNING: ices_get_lineno not implemented");
  return 0;
}
...
playlist_python_get_metadata (void)
{
  ...
  if (pl_get_metadata_hook) {
    if ((res  = python_eval (pl_get_metadata_hook)) && PyString_Check (res))
      ...
      printf("ices_get_metadata failed");
...
python_setup_path (void)
{
  ...
  if (oldpath && (python_path = (char*) malloc (strlen(oldpath) + strlen (ICES_MODULEDIR) + 15)))
    ...
    printf("Could not allocate memory for python environment");
    return -1;
  }
